...
const StarryGalaxy = () => {
  const { showStars } = useContext(AppContext);
      React.useEffect(() => {
        if ((typeof window !== 'undefined')){
          if (!isMobile() && showStars) {
            run()
          }
  ...
  return (
    showStars ? (
      <canvas style={{ zIndex: -1, position: 'fixed', background: '#000000' }} id="canv"></canvas>
    ) : (
        <></>
    )
  )
